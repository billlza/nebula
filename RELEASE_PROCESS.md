...
5. Contract suite passes on the supported matrix. The cited pass/total baseline MUST be
   generated by `scripts/contract_baseline.sh` (never hand-typed) and the same figure used
   in `CHANGELOG.md`, `RELEASE_NOTES_v<version>.md`, and `docs/rc_checklist_v<version>.md`.
